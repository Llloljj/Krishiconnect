  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0">
        <motion.div
          className="absolute left-1/2 top-0 h-96 w-96 -translate-x-1/2 rounded-full bg-primary/10 blur-[100px]"
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {aiCapabilities.map((card) => {
            ...
            return (
              <motion.div
                key={card.title}
                variants={fadeUp}
                className="rounded-3xl border border-white/8 bg-white/[0.04] p-7 backdrop-blur-sm transition hover:border-primary-light/20 hover:bg-white/[0.06]"
                whileHover={{ y: -4 }}
              >
                <div className="flex items-start justify-between">
                  ...
                  <span className="rounded-full bg-primary/20 px-2.5 py-1 text-xs font-bold text-primary-light">
                    {card.metric}
                  </span>
                </div>
        ...
        <motion.div
          className="mt-8 overflow-hidden rounded-3xl border border-white/8 bg-white/[0.04] p-6 shadow-lg shadow-black/20 sm:p-8"
        ...
        >
          <div className="grid gap-6 lg:grid-cols-2">
            <div>
              ...
              <div className="mt-6 h-40 rounded-2xl bg-white/[0.02] p-4 ring-1 ring-white/8">
                <AnimatedBarChart
                  data={[30, 45, 38, 62, 55, 78, 68, 85, 72, 90]}
                  barClassName="bg-gradient-to-t from-primary to-primary-light shadow-[0_0_12px_rgba(129,199,132,0.2)]"
                />
              </div>
            </div>
            <div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                {['Match', 'Forecast', 'Score', 'Alert'].map((label, i) => (
                  <div
                    key={label}
                    className="rounded-2xl bg-white/[0.03] px-4 py-3 text-center ring-1 ring-white/8"
                  >
