...
export default function FPOPreview() {
  return (
    <div className="grid gap-4 lg:grid-cols-12">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.07] p-4 lg:col-span-4 backdrop-blur-sm">
        ...
        <ul className="mt-3 space-y-2 text-sm">
          {['PM-KISAN — 98% enrolled', 'Crop insurance — 76%'].map((s) => (
            <li key={s} className="rounded-xl bg-white/[0.08] px-3 py-2 font-medium text-white">
              {s}
            </li>
          ))}
        </ul>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.07] p-4 lg:col-span-7 backdrop-blur-sm">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.07] p-4 lg:col-span-5 backdrop-blur-sm">
        ...
        <ul className="mt-3 space-y-2">
          {['Active members: 842', 'Pending KYC: 23'].map((row) => (
            <li
              key={row}
              className="flex items-center justify-between rounded-xl bg-white/[0.08] px-3 py-2 text-sm font-medium text-white"
            >
              {row}
            </li>
          ))}
