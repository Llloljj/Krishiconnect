...
export default function FPOPreview() {
  return (
    <div className="grid gap-4 lg:grid-cols-12">
      <div className="grid gap-4 sm:grid-cols-2 lg:col-span-8 lg:grid-cols-4">
        {[
          ...
          return (
            <div
              key={item.label}
              className="rounded-2xl border border-white/8 bg-white/[0.04] p-4 backdrop-blur-sm transition hover:border-white/12 hover:bg-white/[0.06]"
            >
      ...
      <div className="rounded-2xl border border-white/8 bg-white/[0.04] p-4 lg:col-span-4 backdrop-blur-sm">
        ...
        <ul className="mt-3 space-y-2 text-sm">
          {['PM-KISAN — 98% enrolled', 'Crop insurance — 76%'].map((s) => (
            <li key={s} className="rounded-xl bg-white/[0.06] px-3 py-2 font-medium text-white">
              {s}
            </li>
          ))}
        </ul>
      ...
      <div className="rounded-2xl border border-white/8 bg-white/[0.04] p-4 lg:col-span-7 backdrop-blur-sm">
      ...
      <div className="rounded-2xl border border-white/8 bg-white/[0.04] p-4 lg:col-span-5 backdrop-blur-sm">
        ...
        <ul className="mt-3 space-y-2">
          {['Active members: 842', 'Pending KYC: 23'].map((row) => (
            <li
              key={row}
              className="flex items-center justify-between rounded-xl bg-white/[0.06] px-3 py-2 text-sm font-medium text-white"
            >
              {row}
            </li>
          ))}
